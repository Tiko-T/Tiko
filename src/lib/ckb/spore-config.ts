...
// @ts-expect-error The devnet config includes custom script names not covered by the published type.
export const SPORE_CONFIG: SporeConfig<PredefinedDevnetSporeScriptName> = readEnvNetwork() === "testnet" ? predefinedSporeConfigs.Testnet :  {
  lumos: lumosConfig,
  ckbNodeUrl: env.CKB_RPC_URL ?? "http://localhost:28114",
  ckbIndexerUrl: env.CKB_RPC_URL ?? "http://localhost:28114",
  defaultTags: ["latest"],
  scripts: {
    Spore: {
      versions: [
